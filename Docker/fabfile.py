...
def fetch_logs(c, minutes=60, sudo_user="ubuntu"):
    ...
    try:
        ...
        if not container_names:
            print(f"⚠️  No running containers found on {c.host}")
            return
        ...
        for container in container_names:
            ...
            print(f"Fetching logs from {user}@{c.host} for container '{container}' since the last {minutes} minutes")
...
def container_status(c, sudo_user="ubuntu"):
    ...
    print(f"\n📦 Listing Docker containers on {c.user or os.getlogin()}@{c.host}\n")
    try:
