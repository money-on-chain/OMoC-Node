...
class OracleService:
    ...
    async def get_coin_pair_service(self, coin_pair: CoinPair) -> OracleCoinPairService:
        ...
        if coin_pair.is_liquidation_engine():
            coinPairService = self.contract_factory.get_liquidation_engine(coin_pair_info.addr)
        elif coin_pair.is_tasks_runner():
            coinPairService = self.contract_factory.get_tasks_runner(coin_pair_info.addr)
        else:
            coinPairService = self.contract_factory.get_coin_pair_price(coin_pair_info.addr)
