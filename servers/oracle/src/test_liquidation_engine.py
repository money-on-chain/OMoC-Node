...
LOWERCASE_USER = "0xf39fd6e51aad88f6f4ce6ab8827279cfffb92266"
CHECKSUMMED_USER = "0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266"
...
@pytest.mark.asyncio
async def test_liquidation_service_checksums_users_when_publishing():
    encoded_calls = []
    web3_contract = Web3().eth.contract(abi=LIQUIDATION_ENGINE_ABI)

    def execute(method, *args, **kwargs):
        assert method == "runLiquidations"
        encoded_calls.append(
            web3_contract.functions.runLiquidations(*args)._encode_transaction_data()
        )
        return "tx"

    service = LiquidationEngineService(object())
    service.coin_pair_execute = AsyncMock(side_effect=execute)
    params = liquidation_params(
        [PoolLiquidations(POOL_ID, [LOWERCASE_USER])]
    )
    ...
    assert call.args[:6] == (
        ...
        3,
        CoinPair("LENDING").longer(),
        [(POOL_ID, [CHECKSUMMED_USER])],
        ORACLE,
        123,
    )
    assert call.args[6:9] == ([27], [b"r"], [b"s"])
    assert encoded_calls[0].startswith("0x")
...
@pytest.mark.asyncio
async def test_liquidation_service_encodes_lowercase_index_addresses():
    lowercase_liquidation = (
        LOWERCASE_USER,
    ...
    web3_contract = Web3().eth.contract(
        abi=LiquidationEngineService.LENDING_MANAGER_ABI
    )

    class ManagerFunctions:
        @staticmethod
        def isLiquidationAvailable(user, tp_token, moc_bucket):
            encoded_arguments.append((user, tp_token, moc_bucket))
            function = web3_contract.functions.isLiquidationAvailable(
                user, tp_token, moc_bucket
            )
            return SimpleNamespace(
                _encode_transaction_data=function._encode_transaction_data,
                call=lambda transaction: True,
            )

    blockchain = SimpleNamespace(
        get_contract=Mock(
            return_value=SimpleNamespace(functions=ManagerFunctions())
        )
    )
    service = LiquidationEngineService(
        SimpleNamespace(_blockchain=blockchain, addr=MULTICALL)
    ...
    results = await service.get_liquidations_available(
        [lowercase_liquidation], None
    )

    checksummed_liquidation = (
        CHECKSUMMED_USER,
        "0x70997970C51812dc3A010C7d01b50e0d17dc79C8",
        "0x3C44CdDdB6a900fa2b585dd299e03d12FA4293BC",
    )
    assert results == [True]
    assert encoded_arguments == [checksummed_liquidation, checksummed_liquidation]
